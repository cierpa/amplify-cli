...
import { CDKAtmosphereIntegration, EnvironmentDetector } from '../core';
...
describe('CDK Atmosphere Integration', () => {
  ...
  describe('Atmosphere Integration', () => {
      ...
      if (isAtmosphere) {
        ...
        try {
          ...
          if ((error as Error).message.includes('dynamic import')) {
            ...
            throw Error('Parameter issue. The acquire() method expects proper parameters');
          }
        }
      } else {
    ...
    it('should get credentials for Amplify', async () => {
      try {
        const credentials = await integration.getCredentialsForAmplify();

        console.log('✅ Successfully got credentials for Amplify!');
        console.log('Credentials method:', credentials.method);
        console.log('Region:', credentials.region);

        expect(credentials).toBeDefined();
        expect(credentials.region).toBeDefined();
      } catch (error) {
        console.log(`Failed to get credentials for Amplify: ${(error as Error).message}`);
